...
public class PairingInitialisationSequence implements MessageSequencer {
    ...
    private final Logger logger = LoggerFactory.getLogger(PairingInitialisationSequence.class);
    ...
    private @Nullable PairingInitialisationState reTxState;

    private @Nullable Iterator<MaxDevicesHandler> assocIter;
    private @Nullable Iterator<MaxCulWeekProfilePart> weekProfileIter;
    private @Nullable MaxCulWeekProfilePart currentWeekProfilePart;
        ...
        try {
            ...
            switch (state) {
                case INITIAL_PING:
                    ...
                    if (!this.devAddr.equals(msg.srcAddrStr)) {
                        logger.error("Sequence device address {} does not match with message src address {}.",
                ...
                case SENDING_ASSOCIATIONS:
                    if (msg.msgType == MaxCulMsgType.ACK) {
                        ...
                        if (!ack.getIsNack()) {
                            ...
                            Iterator<MaxDevicesHandler> assocIter = this.assocIter;
                            if (assocIter != null && assocIter.hasNext()) /*
                                                                           * this should always be true, but
                                                                           * good to check
                                                                           */ {
                                MaxDevicesHandler partnerCfg = assocIter.next();
                                String partnerRf = partnerCfg.getRfAddress();
                                if (partnerRf != null) {
                                    logger.debug("sendAddLinkPartner for {}: {}", partnerRf, devAddr);
                                    messageHandler.sendAddLinkPartner(this.devAddr, this, partnerRf, MaxCulDevice
                                            .getDeviceTypeFromThingTypeUID(partnerCfg.getThing().getThingTypeUID()));
                                } else {
                                    logger.warn("Could not link to partner {} without rfAddress",
                                            partnerCfg.getThing().getLabel());
                                }
                                /*
                                ...
                                // NOTE: if further states are added then ensure
                        ...
                        if (!ack.getIsNack()) {
                            ...
                            if ((this.deviceType == MaxCulDevice.WALL_THERMOSTAT
                                ...
                                Iterator<MaxCulWeekProfilePart> weekProfileIter = maxDevicesHandler.getWeekProfile()
                                        .getWeekProfileParts().iterator();
                                this.weekProfileIter = weekProfileIter;
                                secondHalf = false;
                ...
                case SENDING_WEEK_PROFILE:
                    if (msg.msgType == MaxCulMsgType.ACK) {
                        ...
                        if (!ack.getIsNack()) {
                            ...
                            MaxCulWeekProfilePart currentWeekProfilePart = this.currentWeekProfilePart;
                ...
                case RETX_WAKEUP_ACK:
                    ...
                    if (msg.msgType == MaxCulMsgType.ACK) {
                        AckMsg ack = new AckMsg(msg.rawMsg);
                        BaseMsg reTxMsg = this.reTxMsg;
                        PairingInitialisationState reTxState = this.reTxState;
                        if (reTxMsg == null || reTxState == null) {
                            logger.error("Ending sequence after WAKEUP: {}", ack.srcAddrStr);
                            state = PairingInitialisationState.FINISHED;
